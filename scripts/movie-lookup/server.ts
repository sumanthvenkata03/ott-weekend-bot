...
// Public origin for canonical URLs, OG tags, robots.txt + sitemap.xml. Override per
// environment (e.g. a preview deploy) via MOVIE_LOOKUP_CANONICAL_ORIGIN; default prod.
const CANONICAL_ORIGIN = process.env.MOVIE_LOOKUP_CANONICAL_ORIGIN ?? "https://thebigscreenindex.com";
...
// The TBSI verdict bridge data (generated by verdicts-export.ts). Read defensively.
const VERDICTS_PATH = join(HERE, "verdict-data", "verdicts.json");
...
const STATIC: Record<string, Asset> = {
  ...
  "/robots.txt": { file: "robots.txt", type: "text/plain; charset=utf-8", cache: HTML_CACHE },
};

async function serveStatic(asset: Asset, res: ServerResponse, transform?: (html: string) => string): Promise<void> {
  try {
    const abs = join(HERE, "public", asset.file);
    // With a transform (OG injection) we read the HTML as text; without one every
    // asset — including binary icons — is served byte-identical as before.
    if (transform) {
      const out = transform(await readFile(abs, "utf8"));
...
/** Escape text for XML (sitemap) attribute/text content. */
function xmlEscape(s: string): string {
  return s.replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&apos;" }[c]!));
}

/**
 * Build sitemap.xml: the static entry pages + one /movie.html?id={id} per film in the
 * now-playing + upcoming India feeds (deduped). Excludes the per-device /watchlist.html.
 * Feed failure still emits the static URLs (never throws). Cached under key "sitemap".
 */
async function buildSitemap(): Promise<string> {
  const ids = new Set<number>();
  try {
    const [now, up] = await Promise.all([movieReleases("now_playing", "IN"), movieReleases("upcoming", "IN")]);
    for (const r of [...now.results, ...up.results]) ids.add(r.id);
  } catch { /* feeds unavailable → still emit the static URLs */ }
  const urls = [
    ...["/", "/compare.html", "/releases.html"].map((p) => CANONICAL_ORIGIN + p),
    ...[...ids].map((id) => `${CANONICAL_ORIGIN}/movie.html?id=${id}`),
  ];
  const body = urls.map((u) => `  <url><loc>${xmlEscape(u)}</loc></url>`).join("\n");
  return `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${body}\n</urlset>\n`;
}

// ── OG / share metadata injection for shareable detail pages ──────────────────
// Light TMDb reads (tmdbGet directly — NEVER movieDetail, which fans out to many
// enrichment calls). Cached; a failure returns null so the caller serves the plain
// file. Every injected value is HTML-escaped at render time (see ogHeadBlock).
function htmlEscape(s: string): string {
  return s.replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c]!));
}

interface OgData { title: string; description: string; ogType: string; imageUrl?: string; }

async function ogMovie(id: number): Promise<OgData | null> {
  try {
    return await apiCache.wrap<OgData | null>(`og:movie:${id}`, async () => {
      const m = await tmdbGet<{ title?: string; original_title?: string; release_date?: string; poster_path?: string | null; original_language?: string }>(`/movie/${id}`);
      const title = m.title ?? m.original_title;
      if (!title) return null;
      const year = m.release_date ? m.release_date.slice(0, 4) : "";
      const lang = langName(m.original_language) ?? "";
      const bits = [year, lang].filter(Boolean).join(" · ");
      const poster = img(m.poster_path, "w500");
      return { title, description: `${bits ? bits + " — " : ""}film record on The Big Screen Index`, ogType: "video.movie", ...(poster ? { imageUrl: poster } : {}) };
    });
  } catch { return null; }
}

async function ogPerson(id: number): Promise<OgData | null> {
  try {
    return await apiCache.wrap<OgData | null>(`og:person:${id}`, async () => {
      const p = await tmdbGet<{ name?: string; profile_path?: string | null }>(`/person/${id}`);
      if (!p.name) return null;
      const profile = img(p.profile_path, "w500");
      return { title: p.name, description: "Filmography & photo gallery — The Big Screen Index", ogType: "profile", ...(profile ? { imageUrl: profile } : {}) };
    });
  } catch { return null; }
}

/** Race the OG fetch against a hard 1.5s budget so a slow TMDb call never blocks the page. */
function ogWithTimeout(kind: "movie" | "person", id: number): Promise<OgData | null> {
  const fetcher = kind === "movie" ? ogMovie(id) : ogPerson(id);
  const timeout = new Promise<null>((resolve) => setTimeout(() => resolve(null), 1500));
  return Promise.race([fetcher, timeout]);
}

/** The canonical + og:* + twitter:* head block (title is replaced separately). */
function ogHeadBlock(og: OgData, canonicalUrl: string): string {
  const e = htmlEscape;
  const lines = [
    `<link rel="canonical" href="${e(canonicalUrl)}" />`,
    `<meta property="og:type" content="${e(og.ogType)}" />`,
    `<meta property="og:site_name" content="The Big Screen Index" />`,
    `<meta property="og:title" content="${e(og.title)}" />`,
    `<meta property="og:description" content="${e(og.description)}" />`,
    `<meta property="og:url" content="${e(canonicalUrl)}" />`,
    ...(og.imageUrl ? [`<meta property="og:image" content="${e(og.imageUrl)}" />`] : []),
    `<meta name="twitter:card" content="summary_large_image" />`,
    `<meta name="twitter:title" content="${e(og.title)}" />`,
    `<meta name="twitter:description" content="${e(og.description)}" />`,
    ...(og.imageUrl ? [`<meta name="twitter:image" content="${e(og.imageUrl)}" />`] : []),
  ];
...
const server = createServer(async (req, res) => {
  try {
    ...
    // OG/title injection for shareable detail pages (movie/person with a numeric id).
    // Best-effort: race the light TMDb read vs 1.5s; any miss (timeout, error, bad/absent
    // id) serves the plain file — never 500, never blocks. App-shell fallback stays plain.
    if (staticKey === "/movie.html" || staticKey === "/person.html") {
      const idParam = url.searchParams.get("id");
      const id = idParam && /^\d+$/.test(idParam) ? Number.parseInt(idParam, 10) : undefined;
      if (id !== undefined) {
        const og = await ogWithTimeout(staticKey === "/movie.html" ? "movie" : "person", id);
        if (og) {
          const canonicalUrl = CANONICAL_ORIGIN + path + url.search;
          const titleTag = `<title>${htmlEscape(og.title)} · The Big Screen Index</title>`;
          const head = ogHeadBlock(og, canonicalUrl);
          const transform = (html: string) => html.replace(/<title>[\s\S]*?<\/title>/, titleTag).replace("</head>", head + "</head>");
          return void (await serveStatic(STATIC[staticKey]!, res, transform));
        }
      }
      return void (await serveStatic(STATIC[staticKey]!, res)); // no id / OG miss → plain file
    }
    ...
    if (seg[0] === "api" && seg[1] === "person" && seg[2]) {
      ...
      // Split endpoint: light profile (default) vs the slow image crawl (/images),
      // separately cached so the person page's first paint never waits on images.
      if (!seg[3]) return sendJson(res, 200, await apiCache.wrap(`person-profile:${id}`, () => personProfile(id)));
      if (seg[3] === "images") return sendJson(res, 200, await apiCache.wrap(`person-images:${id}`, () => personImages(id)));
      return sendJson(res, 404, { error: `no route ${path}` });
    }
    ...
    // ── TBSI verdict bridge: the curated verdicts.json (missing/invalid → empty shape). ──
    if (path === "/api/verdicts") {
      return sendJson(res, 200, await apiCache.wrap("verdicts", async () => safeReadVerdicts(VERDICTS_PATH)));
    }

    // ── SEO: sitemap.xml (not under /api/, so never rate-limited). ──
    if (path === "/sitemap.xml") {
      const xml = await apiCache.wrap("sitemap", () => buildSitemap());
      res.writeHead(200, { "Content-Type": "application/xml; charset=utf-8", "Cache-Control": HTML_CACHE });
